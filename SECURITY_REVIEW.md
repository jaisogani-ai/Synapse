...
**Date:** 2026-06-21
**Status:** v1.0 launch review. Supersedes the prior phase-4 snapshot.
**Verification:** 128 / 128 automated tests passing (39 Rust + 79 Python + 10 TypeScript).

This document is a working threat model. It says what Synapse defends, what it doesn't, what attacks it stops, and which limitations are honest gaps that ship knowingly in v1.0.
...
| **Trust scores** | `TrustStore` (`trust.json` for v1, daemon SQLite next) | Inflated trust → low-rep gate is bypassed → forged messages get less scrutiny. |
...
| Rust daemon IPC bypass | **Mitigated (v1.0.1)** | `SynapseMessage.caps` field; dispatcher checks `is_granted` before every TrustOp. Empty caps → denied with `CAPABILITY_DENIED`. |
...
| Compromised target endpoint URL | **Documented limitation** | `identity_resolver.json` is operator-controlled. If you can write to it, you can redirect messages. v1 assumes the operator's filesystem is trusted. |
| Endpoint hash pinning (defence against above) | **Deferred** (SH-3 in `BUG_REPORT.md`) — v1.x. |

### 2.6 Audit tampering

| Attack | Status | Defence |
|---|---|---|
| Append a fake row | **Mitigated by FS posture** | Audit is append-only JSONL; operator should set `chattr +a` or equivalent. Hash-chaining is a v1.x item (M-9). |
| Delete or rewrite a row | **Not detected in v1** | Documented in `KNOWN_LIMITATIONS.md`. Hash-chained tamper evidence is a planned v1.x item. |
...
| Connection flood on receiver | **Out of scope for v1** | Use the platform's connection limits (systemd, iptables, reverse proxy). Per-sender rate limit is a v1.x item (M-2). |
| Disk fill via inbox spam | **Partially mitigated** | Inbox queues on disk but has no row cap. Manual `synapse inbox` review + reject is the v1 mitigation. |
...
## 4. Fixed in v1.0 / v1.0.1

| Issue | Fix |
|---|---|
| Capability code existed but was not consulted by the IPC dispatcher | **v1.0.1** — wired into both the A2A receiver (`receiver.py`) and the Rust IPC dispatcher (`daemon/src/ipc/mod.rs`). 11 new tests. |
| Marquee demo used a simulated vault, not the real AES-256-GCM one | **v1.0** — `demo.py` now drives `packages/synapse-vault-mcp` via a Node bridge. |
| Inline base64 file transfer capped at 10 MiB; no resume | **v1.0** — A2A-spec `FilePart.uri` form with content-addressed blob endpoint, HTTP `Range` resume, sha256 end-to-end. Cap raised to 2 GiB. |
| Offline target → send fails | **v1.0** — durable SQLite outbox + worker with exponential backoff + DLQ. |
| Daemon banner / `/health` claimed identity, vault, a2a subsystems that didn't exist in the Rust process | **v1.0** — banner and `/health` now report only what's actually running (`trust`). |
| `package.json` workspaces listed an empty dir and a Python pkg | **v1.0** — fixed; `npm install` clean. |
| `~/.cargo/bin/cargo` hardcoded in `package.json` | **v1.0** — relies on PATH. |
| README claimed Phase D complete with stale tree | **v1.0** — full rewrite with accurate module listing. |
| ARCHITECTURE.md described the daemon's IPC as A2A | **v1.0** — corrected; clear disambiguation. |

The full prior bug list lives in [`BUG_REPORT.md`](BUG_REPORT.md). Every CRITICAL and HIGH has been closed.

---

## 5. Known limitations (carry into v1.0)

These are documented gaps that we knowingly ship. Each has either a planned v1.x fix or an operational mitigation.

| Limitation | Mitigation today | Planned fix |
|---|---|---|
| Rust `TrustStore` is in-memory | Python store is v1-authoritative; restart of the Rust daemon loses recorded outcomes. | SQLite backing — v1.x. |
| No end-to-end payload encryption | HMAC integrity only. Use HTTPS / Tailscale / WireGuard for confidentiality. | Documented choice — TLS-or-tunnel is the v1 answer. |
| Endpoint hash pinning (SH-3) | `identity.json` is operator-controlled; assume trusted FS. | Endpoint pinning by sha256 — v1.x. |
| Audit tamper-evidence (M-9) | Append-only JSONL + recommended `chattr +a`. | Hash-chained audit — v1.x. |
| Per-sender rate limit (M-2) | OS / reverse proxy limits. | Add token-bucket per sender — v1.x. |
| Vault at-rest for `vault_client.py` (M-7) | The vault MCP (`packages/synapse-vault-mcp`) is encrypted at rest. The CLI's in-process `vault_client.py` is in-memory only, used for sender-side proxy issuance during a single CLI invocation. | Encrypt-at-rest for `vault_client.py` — v1.x. |
| Inbox SQLite WAL/busy-timeout (M-1) | Outbox already uses WAL; inbox does not. Low concurrency in v1, so this is mostly cosmetic. | Enable on inbox — v1.x. |
...
4. **The Python store is the canonical trust store for v1.** The Rust store is a stub being filled in over the v1.x line. Do not split-brain configure both.
