...
- **Patch review workflow** — reviewing agents return unified diffs (`make_patch`, stdlib `difflib`); senders apply them with strict context validation (`apply_patch` refuses a stale/mismatched patch rather than half-applying); `PatchThread` models the comment → revise → resubmit loop keyed by A2A `context_id`. CLI: `synapse patch make / summarize / apply [--dry-run]`. (`patch.py`)
