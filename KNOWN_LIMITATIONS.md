...
4. **No multi-tenant SaaS.** Self-hosted only. v0.1 has no notion of "tenant," "organisation," or "billing."
...
|---|---|---|---|---|
| C-1 | No end-to-end payload encryption | A network observer can read A2A JSON-RPC payloads (not raw vault secrets — those never serialize) | Put HTTPS / Tailscale / WireGuard / SSH tunnel underneath | Documented choice — TLS-or-tunnel is the v0.1 answer. If we add encryption later it will be opt-in. |
...
| # | Limitation | Impact | Mitigation today | Plan |
|---|---|---|---|---|
| T-1 | Rust `TrustStore` is in-memory | Daemon restart loses recorded outcomes | Python store (`trust.json`) is v0.1-authoritative and is persisted | SQLite backing for Rust store — v0.2 |
| T-2 | Two trust stores exist (Python + Rust); only the Python one is authoritative | If you query the Rust daemon over IPC for `get_score`, the answer comes from a different store than the one the CLI uses | Read the docs: Python is canonical for v0.1 | Collapse to one store — v0.2 |
| T-3 | No trust score decay over time | A score earned in 2024 weighs the same as one earned today | Operator can manually reset | Time-decay weighting — v0.2 |
...
| # | Limitation | Impact | Mitigation today | Plan |
|---|---|---|---|---|
| V-1 | `vault_client.py` is in-memory only | A process restart of the sender CLI loses any locally-stored secrets | Use the vault MCP for any persisted secret; `vault_client.py` is for in-flight proxy issuance during a single CLI run | Encrypt-at-rest for `vault_client.py` — v0.2 (M-7) |
| V-2 | Vault master key is generated per-process | Restart the vault MCP, lose access to previously-stored ciphertext | Persist the master key out-of-band (env var, KMS, etc.) when running in production | Documented integration path; no change in v0.1 |
| V-3 | Proxy TTL max is 1 hour (`MAX_PROXY_SECONDS`) | Long-running deploys must re-issue proxies | Re-request periodically | Configurable — v0.2 |
...
| # | Limitation | Impact | Mitigation today | Plan |
|---|---|---|---|---|
| I-1 | Inbox SQLite is not WAL + busy-timeout (M-1) | Concurrent writers could see `database is locked` | One process per host is the v0.1 default — not a real issue | Enable WAL on inbox (outbox already has it) — v0.2 |
| I-2 | No per-sender rate limit (M-2) | A flood of valid messages from one sender fills the inbox | OS / reverse proxy limits | Token-bucket per sender — v0.2 |
...
| # | Limitation | Impact | Mitigation today | Plan |
|---|---|---|---|---|
| N-1 | `identity.json` is operator-controlled, no endpoint hash pinning (SH-3) | Whoever writes the file can redirect messages | Restrict file permissions on `$SYNAPSE_HOME` | Endpoint pinning by sha256 — v0.2 |
...
| N-3 | No connection pooling | Each `post_jsonrpc` opens a new TCP connection | Latency is the cost, not correctness | HTTP keep-alive — v0.2 |
...
| # | Limitation | Impact | Mitigation today | Plan |
|---|---|---|---|---|
| K-1 | Capability vocabulary lives in two places (Python and Rust) | Must be kept in sync by hand | Compared in code review; same set | Code-gen one from the other — future consideration |
...
|---|---|---|---|
| P-1 | Tested on macOS (Darwin 25.5) | Linux should work; Windows is untested | Use Linux/macOS for v0.1 |
...
| # | Limitation | Impact | Mitigation today | Plan |
|---|---|---|---|---|
| H-1 | No CI workflow in `.github/workflows/` | Tests must be run locally before push | Run `cargo test && pytest && npm test` before every commit | Add GitHub Actions — v0.2 |
| H-2 | No release automation | Tags published by hand | Maintainer cuts each release manually | Add release-please / similar — v0.2 |
| H-3 | No SBOM | Downstream consumers can't audit dependency tree mechanically | `cargo tree`, `pip list`, `npm ls` work | Generate CycloneDX SBOM on release — v0.2 |
...
- **"Synapse uses HMAC, not Ed25519."** True, and intentional — HMAC has no PKI requirement. Asymmetric is a future consideration if it earns its way in.
- **"Synapse stores trust as JSON on disk."** Yes — small surface, easy to inspect, easy to back up. Rust SQLite store is a future parallel option.
